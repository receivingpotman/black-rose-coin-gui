...
void SendFrame::amountValueChange() {
    ...
    Q_FOREACH (TransferFrame * transfer, m_transfers) {
      quint64 amount = CurrencyAdapter::instance().parseAmount(transfer->getAmountString());
      quint64 percentfee = amount * 0.01 / 100; // fee is 0.01%
    ...
    Q_FOREACH (TransferFrame * transfer, m_transfers) {
      ...
      float donationpercent = amount * 0.0 / 100; // donation is 0.0%
...
void SendFrame::parsePaymentRequest(QString _request) {
    ...
    if(_request.startsWith("blackrose://", Qt::CaseInsensitive))
    {
       _request.replace(0, 7, "blackrose:");
    }
    if(!_request.startsWith("blackrose:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with blackrose:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "br221LrvUWdbfsnbLTTW55eYnyQhXyX26fvSEDkDwGTYKQsKeuHPWJJGzLwY9pLLXX7N1kwyAzTBzByN1ECTGZx617hh9amoN";
...
void SendFrame::sendAllClicked() {
  ...
  if (m_ui->donateCheckBox->isChecked()) {
    float donation_amount = CurrencyAdapter::instance().formatAmount(amount).toDouble() * 0.0 / 100;
    donation_amount = floor(donation_amount * pow(10., 4) + .5) / pow(10., 4);
    float min = getMinimalFee();
    if (donation_amount < min)
        donation_amount = min;
