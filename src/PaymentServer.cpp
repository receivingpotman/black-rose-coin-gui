...
const QString BITCOIN_IPC_PREFIX("br1:");

static QString ipcServerName()
{
    QString name("BlackRose");
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start blackrose: click-to-pay handler");
    else
...
{
    // clicking on blackrose: URLs creates FileOpen events on the Mac:
